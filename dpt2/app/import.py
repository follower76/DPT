...
#		os.mkdir(destpath)
		...
		for row in range(9, sheet.nrows):
			restaurant = Restaurant(version=version, oid=sheet.cell(row, 0).value, name=sheet.cell(row, 1).value, link=sheet.cell(row,2).value)#,menu=sheet.cell(row,6))
			restaurant.save()
		...
		for row in range(rowStart + 1, sheet.nrows):
			...
			product = Product(version=version, oid=sheet.cell(row, 1).value, name=sheet.cell(row, 2).value, restaurant=Restaurant.objects.get(oid=sheet.cell(row, 3).value, version=version))#get(oid=sheet.cell(row, 3).value))
...
#		sheet = workbook.sheet_by_index(0)
		for row in range(rowStart + 1, sheet.nrows):
			...
			for col in range(ncolumns + 1, ncolumns_util):
				function_property = FunctionProperty(function=function, property=properties[col - (ncolumns + 1)], value=sheet.cell(row, col).value)
...
#############################################
		...
		matrix_Obj = BigMatrix(version=version)
		matrix_Obj.matrix = []
#		stimuli_col = ncolumns_util + (num_stimuli + 1)		
		...
		for i in range(1,num_stimuli + 1): # Ranges for stimuli 
			for j in range(i+1,num_stimuli + 1):
				index += 1 
				matrix_Obj.matrix.append([])
				for k in range(1,num_functions+1):
					matrix_Obj.matrix[index].append(int(sheet.cell(k+rowStart,stimuli_col+j).value) - int(sheet.cell(k+rowStart,stimuli_col+i).value)) # Data will be coming from excel file rank(j) - rank(i)
		#matrix_Obj.save()
		f=open('dpt2Matrix.py', 'w+')
		f.write('Big_Matrix='+str(matrix_Obj.matrix))
		f.close() 
